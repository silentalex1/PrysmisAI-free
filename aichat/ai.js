(function () {
  ...
  function init() {
    ...
    if (!session || !session.username) {
      window.location.href = 'https://prysmisai-free.onrender.com/auth/index.html';
      return;
    }
    showApp();
  ...
  var OLLAMA_MODELS = [
    { value: 'PrysmisAI', label: 'PrysmisAI' }
  ];

  function getSelectedModel() {
    return localStorage.getItem('prysmis_model') || 'PrysmisAI';
  }
  ...
  async function simulatePrysmisAI(prompt) {
    var conversationalResponses = [
      "I'd be happy to help you with your Roblox development! What specific aspect are you working on?",
      "That's a great question! Could you tell me more about what you're trying to accomplish?",
      "I can definitely assist with that. Are you looking for a specific type of script or system?",
      "Interesting! Let me think about the best approach for your project. What's your current setup?",
      "I'd love to help you build this! Do you have any particular requirements or preferences?",
      "That sounds like an exciting project! Are you working on a game, tool, or something else?",
      "I can help you with that! Are you looking for something simple or more advanced?",
      "Great choice! What experience level are you targeting for your players?",
      "I'd be glad to assist! Do you need help with the scripting, UI, or game mechanics?",
      "That's a solid idea! Are there any specific features you want to include?"
    ];
    
    var codeResponseResponses = [
      "I can help you with the code for that! Here's a script that should work:",
      "Let me write that code for you. This should do what you need:",
      "Here's a solution I think you'll find helpful:",
      "I can provide a script for that. Try this:",
      "Let me create the code for your project:"
    ];
    ...
    var shouldProvideCode = Math.random() > 0.7;
    
    if (shouldProvideCode) {
      var responseText = codeResponseResponses[Math.floor(Math.random() * codeResponseResponses.length)];
      var randomCode = codeExamples[Math.floor(Math.random() * codeExamples.length)];
      return {
        response: responseText + "\n\n" + randomCode,
        code: randomCode
      };
    } else {
      var responseText = conversationalResponses[Math.floor(Math.random() * conversationalResponses.length)];
      return {
        response: responseText,
        code: null
      };
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    try {
      ...
      var lastUserMessage = msgHistory[msgHistory.length - 1];
      var userPrompt = lastUserMessage ? (typeof lastUserMessage.content === 'string' ? lastUserMessage.content : JSON.stringify(lastUserMessage.content)) : '';
      ...
      var aiResult = await simulatePrysmisAI(userPrompt);
      streamedText = aiResult.response;
  ...
  window.openSettings = function () {
    ...
    var keyInp = document.getElementById('deepinfra-key-input');
    if (keyInp) {
      var savedKey = getDeepinfraKey();
      keyInp.value = savedKey;
      keyInp.type = 'password';
      var toggleBtn = document.getElementById('deepinfra-key-toggle');
  ...
  window.openShareScreen = function() {
    ...
    var url = 'https://prysmisai-free.onrender.com/screenshare/index.html?token=' + encodeURIComponent(token);
    window.open(url, 'PrysmisAI_ScreenShare', 'width=1400,height=900,toolbar=no,menubar=no,scrollbars=no,resizable=yes');
  };
