(function () {
  ...
  function getGeminiKey() {
    return localStorage.getItem('prysmis_gemini_key') || '';
  }
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    try {
      ...
      if (!apiKey) {
        aiRowBubble.innerHTML = '<span style="color:#e05555">No Gemini API key set. Go to Settings &rarr; AI Settings to add your key.</span>';
        hideChecklist();
        generating = false;
        setStatus('ready');
      ...
      var systemInstruction = { parts: [{ text: getSystemPrompt() }] };
      ...
      if (!response.ok) {
        ...
        var isQuota = response.status === 429 ||
          ...
          errMsg.toLowerCase().indexOf('resource_exhausted') !== -1;

        if (isQuota) {
          showQuotaToast();
          aiRowBubble.innerHTML = '<span style="color:#e05555">Your Gemini API key quota has been exhausted.</span>';
        } else {
