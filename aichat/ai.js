(function () {
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    try {
      ...
      var ollamaResp = await fetch('/api/chat', {
        method: 'POST',
        headers: {
      ...
      if (!ollamaResp.ok) {
        var errorData = await ollamaResp.json().catch(function() { return {}; });
        throw new Error(errorData.error || 'PrysmisAI error ' + ollamaResp.status);
      }
