(function () {
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    try {
      ...
      var ollamaMessages = [{ role: 'system', content: getSystemPrompt() }];
      msgHistory.forEach(function(m) {
        if (Array.isArray(m.content)) {
          ...
          ollamaMessages.push({ role: m.role, content: typeof m.content === 'string' ? m.content : JSON.stringify(m.content) });
        }
      });
      ...
      var ollamaResp = await fetch('http://127.0.0.1:11434/api/generate', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          model: selectedModel,
          prompt: getSystemPrompt() + '\n\n' + formatMessagesForOllama(ollamaMessages),
          stream: true,
          options: { num_predict: 4096, temperature: 0.7 }
        })
      }).catch(function() {
        throw new Error('PrysmisAI requires Ollama to be running locally. Please install Ollama and run: npm run setup-ollama');
      });

      if (!ollamaResp.ok) {
        throw new Error('PrysmisAI error ' + ollamaResp.status);
      }

      var reader = ollamaResp.body.getReader();
