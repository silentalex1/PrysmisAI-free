(function () {
  ...
  function getOpenRouterKey() {
    return localStorage.getItem('prysmis_openrouter_key') || '';
  }
  ...
  function showQuotaToast() {
    var toast = document.getElementById('quota-toast');
    if (!toast) return;
  ...
  window.saveGeminiSettings = function () {
    ...
    var st = document.getElementById('gemini-settings-status');
    if (!val) {
      if (st) { st.textContent = 'Please enter an API key.'; st.style.color = '#e05555'; }
      return;
    }
    localStorage.setItem('prysmis_openrouter_key', val);
    if (st) { st.textContent = 'Settings saved.'; st.style.color = '#4caf7d'; setTimeout(function () { st.textContent = ''; }, 2500); }
  };
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    if (checkItems) runChecklistAnimation(checkItems, null);

    try {
      var apiKey = getOpenRouterKey();
      if (!apiKey) {
        aiRowBubble.innerHTML = '<span style="color:#e05555">No OpenRouter API key set. Go to Settings &rarr; AI Settings to add your key.</span>';
        hideChecklist();
        generating = false;
        setStatus('ready');
      ...
      if (!response.ok) {
        ...
        var isQuota = response.status === 429 ||
          ...
          errMsg.toLowerCase().indexOf('insufficient') !== -1;

        if (isQuota) {
          showQuotaToast();
          aiRowBubble.innerHTML = '<span style="color:#e05555">Your OpenRouter API key has run out of credits.</span>';
        } else {
