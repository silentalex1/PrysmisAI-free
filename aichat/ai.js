(function () {
  ...
  window.saveGeminiSettings = function () {
    ...
    localStorage.setItem('prysmis_poe_key', val);
  ...
  async function runAiRequest(checkItems, isContinue) {
    ...
    try {
      var apiKey = getPoeKey();
      if (!apiKey) {
        aiRowBubble.innerHTML = '<span style="color:#e05555">No Poe API key set. Go to Settings &rarr; AI Settings to add your key.</span>';
        hideChecklist();
        generating = false;
        setStatus('ready');
