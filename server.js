...
const OLLAMA_MODEL = 'PrysmisAI';

async function ollamaStream(model, messages, res) {
  const response = await fetch(`${OLLAMA_BASE_URL}/api/generate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, messages, stream: true, options: { num_predict: 4096, temperature: 0.7 } })
  });
  if (!response.ok) {
...
async function ollamaComplete(model, messages) {
  const response = await fetch(`${OLLAMA_BASE_URL}/api/generate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, messages, stream: false, options: { num_predict: 4096, temperature: 0.7 } })
  });
  if (!response.ok) {
    const errText = await response.text().catch(() => '');
    throw new Error('Ollama error ' + response.status + ': ' + errText.slice(0, 200));
...
async function streamOllamaToRes(model, messages, res) {
  const ollamaResp = await ollamaStream(model, messages, res);
  const reader = ollamaResp.body;
  let buffer = '';
...
app.post('/api/chat', async function(req, res) {
  ...
  try {
    ...
    await streamOllamaToRes(OLLAMA_MODEL, ollamaMessages, res);
  } catch (err) {
...
app.post('/api/studio/connect', function(req, res) {
  ...
  return res.json({ success: true, username: session.username, model: OLLAMA_MODEL });
});
...
app.post('/api/studio/animate', async function(req, res) {
  ...
  const animApiKey = req.body.apiKey || null;
  try {
    ...
    let code = await ollamaComplete(OLLAMA_MODEL, msgs);
...
app.post('/api/studio/screen-chat', async function(req, res) {
  ...
  const chatApiKey = req.body.apiKey || null;
  try {
    ...
    let fullResponse = await ollamaComplete(OLLAMA_MODEL, msgs);
    let code = null;
